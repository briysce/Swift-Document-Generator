...
| 10 | Swift rebuild details | | done | grey bar edge = the 1.4 px border's anti-aliasing (already drawn); anchors already lean (≈13 segments per SWIFT letter, ≈14 per SUPPLY letter, 230 for the whole outline+shadow) — nothing left worth trimming |
...
| 16 | Corpus v2: the synthetic degrader turns neutral grey ink transparent and keeps the white page (Trialta loses its fan, TRI and PROJECTS before the engine runs; E0134). Version the degrader, put its version in the corpus fingerprint, regenerate, re-baseline both engines on it — never a silent swap | claude | in progress | v2 built (`a56200d`); baseline run on pairs_v2.json next | after spec B finishes; `scripts/logo_synthetic_degrade.py::_jpeg_reconstruct_alpha` |
