...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "status":
        ...
        print("2. Link WhatsApp once on a normal PC/home network (not cloud/AWS):")
        print("   python -m tools.meedo_me.runtime gateway channels login --channel whatsapp")
        print("   Scan QR in WhatsApp → Linked devices within ~15s of each refresh.")
        print("   Cloud hosts often fail with WhatsApp's \"Couldn't Link device\" — that is")
        print("   egress IP rejection, not a Meedo bug. See tools/meedo_me/runtime/openclaw/README.md")
