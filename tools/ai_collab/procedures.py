...
class Procedure:
    ...
    # Why this procedure must not be taught (e.g. written by a test fixture).
    # Kept, not deleted: the union merge would bring a deleted one back.
    retracted: str = ""
...
def recall_procedures(
    ...
    *,
    ...
    for proc in rows:
        if proc.retracted or (face and proc.face != face):
            continue
...
def offline_ready_summary(path: Path | None = None) -> dict[str, Any]:
    rows = [r for r in load(path) if not r.retracted]
